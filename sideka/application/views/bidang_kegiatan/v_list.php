...
function edit_bidang_kegiatan(id) {
  window.location = _base_url + 'datapenduduk/c_bidang_kegiatan/edit/' + id;
}
...
function btn(com,grid)
{
	...
	if (com=='Add')
    {
		window.location = _base_url + 'datapenduduk/c_bidang_kegiatan/add';
    }
	...
	if (com=='Delete Selected Items')
        {
           if($('.trSelected',grid).length>0){
			   if(confirm('Hapus ' + $('.trSelected',grid).length + ' item?')){
					...
					$.ajax({
					   type: "POST",
					   url: "<?=site_url("datapenduduk/c_bidang_kegiatan/delete/");?>",
					   data: "items="+itemlist,
...
<!-- <div class="col-md-3">
	...
	<li class="dropdown">
		...
		<ul class="dropdown-menu dropdown-user">
			<li>
				<a href="<?php echo site_url('datapenduduk/c_bidang_kegiatan/ExportToExcel/');?>"><i class="fa fa-download fa-fw"></i> Export ke Excel</a>
			</li>
		</ul>
	</li>
</ul>
